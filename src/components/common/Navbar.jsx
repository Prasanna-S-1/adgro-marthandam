...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <div className={`hidden lg:block w-full bg-brand-dark transition-all duration-500 overflow-hidden ${isScrolled ? 'h-0 opacity-0' : 'h-[40px] opacity-100'}`}>
        <div className="container mx-auto px-6 max-w-7xl h-full flex justify-between items-center text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400">
          ...
          <div className="flex items-center gap-6">
            <a href="mailto:adgromarthandam@gmail.com" className="hover:text-brand-red transition-colors">
              adgromarthandam@gmail.com
            </a>
            <a href="tel:+919783156789" className="flex items-center gap-2 text-white hover:text-brand-red transition-colors">
      ...
      <nav className={`fixed w-full z-[100] transition-all duration-500 border-b ${
        isScrolled 
          ? 'top-0 bg-white/90 backdrop-blur-xl py-4 shadow-premium border-gray-100' 
          : 'top-0 lg:top-[40px] bg-white py-6 border-transparent'
      }`}>
        <div className="container mx-auto px-6 max-w-7xl flex justify-between items-center">
          
          {/* 1. LOGO AREA (With Temporary Image Slot) */}
          <a href="/" className="flex items-center gap-4 group relative z-50">
            {/* TEMPORARY IMAGE SLOT: Replace src with your official logo URL later */}
            <div className="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center overflow-hidden border border-gray-200 shadow-sm group-hover:border-brand-red transition-colors duration-300">
              <img 
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link, index) => (
              ...
              >
                <a 
                  href={link.path}
                  className="flex items-center gap-1 text-[13px] font-bold uppercase tracking-widest text-brand-dark hover:text-brand-red transition-colors py-2"
                >
                  {link.name}
                  {link.dropdown && (
                  ...
                  <AnimatePresence>
                    {activeDropdown === index && (
                      <motion.div
                        ...
                        className="absolute top-full left-1/2 -translate-x-1/2 w-[280px] bg-white shadow-premium-hover rounded-xl border border-gray-100 overflow-hidden pt-2 pb-2"
                      >
                        ...
                        <div className="flex flex-col">
                          {link.dropdown.map((subItem, subIndex) => (
                            <a 
                              key={subIndex} 
                              href={subItem.path}
                              className="px-6 py-3 text-sm font-sans text-gray-600 hover:text-brand-red hover:bg-gray-50 flex items-center justify-between group/item transition-all"
                            >
                              {subItem.name}
            ...
            {/* DESKTOP CTA BUTTON */}
            <a href="#book" className="btn-premium px-8 py-3 text-[13px] ml-4 shadow-clinical hover:shadow-premium-hover">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 bg-white z-[90] lg:hidden flex flex-col pt-[100px] pb-6 px-6 overflow-y-auto"
          >
                        ...
                        >
                          {link.dropdown.map((subItem, subIndex) => (
                            <a 
                              key={subIndex} 
                              href={subItem.path}
                              className="text-gray-500 font-sans text-lg flex items-center gap-2"
                            >
            ...
            >
              <a href="#book" className="btn-premium w-full py-4 text-center justify-center text-sm shadow-clinical">
                <span className="relative z-10">Book Appointment</span>
              </a>
